...
export const userReducer = (state = initialUserState, action: UserActions): UserData => {
  switch (action.type) {
    ...
    case UserActionTypes.LoadUserMenu:
      return { ...state, usermenu_loading: true };

      case UserActionTypes.LoadUserMenuSuccess: {
        return { ...state, usermenu_loading: false, usermenu: action.payload };
      }
    ...
    case UserActionTypes.LoadUserMenuFail: {
      return { ...state, usermenu_loading: false, error: action.payload };
    }
