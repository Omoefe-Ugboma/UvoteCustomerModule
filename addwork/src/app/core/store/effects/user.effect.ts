  ...
  users$ = createEffect(() =>  this.actions$.pipe(
    ...
    mergeMap(() => this.userService.getUsers()
      .pipe(
        map((data) => {
          if (data.status === true) {
            return new userActions.LoadUsersSuccess(data.data.data);
          }
          return new userActions.LoadUsersFail(getErrorObj(data));
        }),
        catchError(error => of(new userActions.LoadUsersFail(getErrorObj(error))))
      )
    )
  ));
